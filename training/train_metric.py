"""
Train ConvNeXt-Base with ArcFace + Triplet loss for tree re-identification.

Phase 1 (epochs 1-3): Freeze backbone, train head only
Phase 2 (epochs 4+):  Unfreeze all, differential LR

Run: python training/train_metric.py
"""
...
def compute_recall_at_k(embeddings, labels, k_values=[1, 5]):
    """Compute recall@k on embeddings. For each query, check if nearest neighbor has same label."""
...
def validate(model, val_dataset, device, batch_size=64):
    """Extract embeddings and compute recall@1,5."""
    model.eval()
...
def train():
    # Config
    PRETRAINED_CKPT = r'E:\tree_id_new\tree_id\output\models\tree_reid_convnext\best_model.pth'
    DATA_DIR = r'E:\tree_id_2.0\data'
    IMAGE_BASE = r'E:\tree_id_2.0\images'
    OUTPUT_DIR = r'E:\tree_id_2.0\models\metric'
    ...
    train_dataset = TreeMetricDataset(excel_path, IMAGE_BASE, encoder_path, split='train')
    val_dataset = TreeMetricDataset(excel_path, IMAGE_BASE, encoder_path, split='val')
    ...
    # Model - load pretrained backbone + embedding head
    print(f"\nLoading pretrained ConvNeXt-Base from {PRETRAINED_CKPT}")
    model = TreeReIdModel.from_pretrained_partial(PRETRAINED_CKPT, device=str(device))
    model.train()
    ...
    print(f"\n{'='*60}")
    print(f"Training: {EPOCHS} epochs, P={BATCH_P} K={BATCH_K}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch == FREEZE_EPOCHS + 1:
            ...
            model.unfreeze_all()
            # Rebuild optimizer with backbone params now requiring grad
            optimizer = torch.optim.AdamW([
        ...
        if epoch <= WARMUP_EPOCHS:
            ...
            lr_scale = max(lr_scale, 0.01)  # min 1% of base LR
        ...
        if epoch % 2 == 0 or epoch == EPOCHS:
            recall = validate(model, val_dataset, device)
            r1 = recall['recall@1']
            r5 = recall['recall@5']
